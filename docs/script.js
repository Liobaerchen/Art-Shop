...
(function () {
  ...
  // Uses the smaller, phone-sized WebP set below the same 1080px breakpoint
  // the grid layout itself switches at (see the @media rule in style.css),
  // and the larger desktop set above it. Converted by optimize_images.py —
  // run that first, or these paths won't have anything to load.
  var IS_MOBILE_GRID = window.matchMedia('(max-width: 1080px)').matches;
  var BASE = 'files/' + (IS_MOBILE_GRID ? 'about-mobile' : 'about-desktop') + '/';

  // optimize_images.py always outputs .webp, replacing whatever the original
  // extension was — this mirrors that so the JS path matches the real file.
  function toWebp(path) { return path.replace(/\.[^./]+$/, '.webp'); }
  ...
  function runCategory(catIndex, onDone) {
    ...
    for (var k = 0; k < n; k++) {
      ...
      img.src = BASE + toWebp(chosenFiles[k]);
